...
- 金属薄板はチューブ本体と別マテリアルにし、初期仮値を色 `#ffc0c0`、metalness 1、roughness 0.4とする
- 表面には人工的な加工として読めるパターンを持たせる。全面を粗面化せず、鏡面部分を残して粗さを切り替える
- 金属薄板の加工パターンはTSLで生成する。面内を10×3のグリッドに分け、基準面のmetalness 1・roughness 0.4と、交互セルのmetalness 0.5・roughness 0.6を市松状に配置する。市松の配置を維持したまま各セルの横方向境界を曲げ、中央が先端方向へ張り出す鱗形とする。外周枠は設けない
...
金属の種類、最終的な表面仕上げ、具体的な構図は未決定。

### 初期実装の仮値

- 背景色: `#00070c`、フォグ密度: 0.055
- チューブ本体: 色 `#004080`、metalness 0.3、roughness 0.5
- 金属薄板: 色 `#ffc0c0`、基準面はmetalness 1・roughness 0.4、交互セルはmetalness 0.5・roughness 0.6
