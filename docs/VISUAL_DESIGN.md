...
自然光や空間全体を照らす環境光は想定しない。観測機器由来の探照灯が通る範囲だけで水中散乱が見え、遠方は吸収されて黒へ消える状態を目指す。初期実装では、探照灯と同じ向きに半透明のコーンメッシュを置き、独立した光体積として近似する。マリンスノーの不透明度は光体積によって変えない。
...
- 光体積: 色`#9edbea`、不透明度0.045、距離22m、角度0.48、放射分割32、長手分割8の半透明コーン
